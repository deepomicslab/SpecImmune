...
class VCFTrack(genomeview.IntervalTrack):
    def __init__(self, vcf_path, name=None):
        ...
        self.row_height = 15
        self.min_variant_pixel_width = 1
        ...
        for rec in self.vcf:
            ...
            _interval_id += 1
            interval = genomeview.Interval(interval_id, rec.chrom, rec.start, rec.stop+1, None)
...
source = genomeview.FastaGenomeSource("HLA-C.2.fasta")
view = genomeview.GenomeView("HLA-C_ref2", 0, 500, "+", source)
doc.add_view(view)

view.add_track(genomeview.BAMCoverageTrack("HLA-C.1.bam", name="bam coverage"))
...
# bam_track_hg002 = genomeview.SingleEndBAMTrack("HLA-C.1.bam", name="HLA-A hapotype 0")
...
genomeview.save(doc, "example.doc.pdf")