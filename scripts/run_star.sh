...
threads=1
src=/scratch/project/cs_shuaicli/wxd/app/SpecLong/
reads_dir=/scratch/project/cs_shuaicli/wxd/cyp2d6_amp/CYP2D6
outdir=/scratch/project/cs_shuaicli/wxd/cyp2d6_amp/CYP2D6/amp_speclong_out
REFERENCE_GENOME="/scratch/project/cs_shuaicli/1KG_ONT_VIENNA_ref/1KG_ONT_VIENNA_hg38.fa"
db=/scratch/project/cs_shuaicli/wxd/app/SpecLong/db/
if [[ ! -d "${outdir}" ]]; then
    mkdir -p "${outdir}"
fi
...
process_sample() {
    ...
    fq=$reads_dir/$sample_id.fastq.gz
    ...
    python extract_clip.py $bam $extract_reads 200 false
