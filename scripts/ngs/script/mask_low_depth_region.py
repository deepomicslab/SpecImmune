...
class Mask_low():
    ...
    def select_focus_interval(self, depth_list, exon_intervals):
        ...
        if self.focus_exon == True:
            ...
            mask_region = self.get_low_region(depth_list, mask_region, 300, len(depth_list)-300)
        return mask_region


    def read_exons(self, gene):
        exon_bed = "%s/../db/%s/%s.ref.exon.extend.txt"%(sys.path[0], self.geneclass, self.geneclass)
        exon_intervals = []
        f = open(exon_bed, 'r')
        for line in f:
            array = line.strip().split()
...
if __name__ == "__main__":
    ...
    required.add_argument("-g", type=str, help="gene class [KIR|HLA|CYP]", metavar="\b")
