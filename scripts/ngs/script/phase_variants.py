...
def read_vcf(vcffile,outdir,snp_dp,bamfile,indel_len,gene,freq_bias,\
    ...
    for record in in_vcf.fetch():
        ...
        if record.chrom != gene:
            continue
        if record.chrom == 'HLA-DRB1' and record.pos >= 3198 and record.pos <= 3700:
            continue
        # print (record, dp)
...
def freq_output(outdir, gene, fresh_alpha, hete_var_num):
    ...
    for j in range(len(fresh_alpha)):
        print ('allele.%s.%s.fasta'%(str(j+1), gene),fresh_alpha[j],file=ra_file)
...
class MNP_linkage():
    ...
    def get_discard_reads(self):
        if gene == "HLA-DRB1":
            for read in self.samfile.fetch("HLA-DRB1", 3198, 3700 ): #3198, 3700  3955, 4251
...
def block_phase(outdir,seq_list,snp_list,gene,gene_vcf,rephase_vcf,record_block_haps):
    ...
    for record in m.fetch():
        ...
        #    else:
        #        phased_locus=seq[he]
        #        print(record.pos, phased_locus, he)
        if geno != (0,0) and geno != (1,1) and geno != (2,2):
            if geno == (1,2) or geno == (2,1):
                ...
                phased_locus=seq[he]
                #print(record.pos, phased_locus, geno, he)
...
class Share_reads():
            ...
            for read in samfile.fetch(self.gene, region[0] - 1, region[1]):
                ...
                if rivet_points==True:
                    ...
                    if hap_belong != 'NA':
                        ...
                        if self.gene == "HLA-DRB1" and read.reference_start < 3988: 
    ...
    def split_seg(self):
        ...
        for seg in segs:
            ...
            if seg[2] == 'insertion':
                continue
            if self.gene == 'HLA-DRB1' and seg[0] < 3198 and seg[1] > 3700:
                contain_dup_seg = seg
                seg[2] = 'dup'
                seg_region = ' %s:%s-3198 %s:3198-3700 %s:3700-%s '%(self.gene,\
                seg[0],self.gene,self.gene,seg[1])
                seg_region_front = ' %s:%s-3198 '%(self.gene, seg[0])
                seg_region_dup = ' %s:3198-3700 '%(self.gene)
                seg_region_behind = ' %s:3700-%s '%(self.gene,seg[1])
            ...
            gap += seg_region
        for i in range(self.strainsNum):
            order='%s/../bin/samtools faidx %s/../db/immune.complex.gene.ref.extend.fasta\
            ...
            hap_seq =  '>%s_%s\n'%(self.gene, i) + hap_seq[:]
            out = open('%s/allele.%s.%s.fasta'%(self.outdir,i+1,self.gene), 'w')
            print (hap_seq, file = out)
            out.close()

    def link_diploid_insertion(self, insertion_seg):
        # insertion_seg = 'HLA-DRB1_6355'
...
def long_InDel_breakpoints(bfile):
    ...
    for line in f:
        ...
        if array[0] != array[3]:
            continue
        if array[0] == 'HLA-DRB1':
            if int(array[1]) > 3100 and int(array[1]) < 3800:
                continue
            if int(array[4]) > 3100 and int(array[4]) < 3800:
                continue
...
def get_unphased_loci(outdir, gene, invcf, snp_list, spec_vcf):
    ...
    for record in m.fetch():
        ...
        geno = record.samples[sample]['GT']
        depth = record.samples[sample]['AD']
        if geno == (1,1) or geno == (0,0) or geno == (2,2):
            record.samples[sample].phased = True
            out.write(record)
            continue
        if geno != (1,1) or geno != (0,0) or geno != (2,2):
        ...
        if record.samples[sample].phased == True:
            if ptag > 0 :
                record.samples[sample]['PS'] +=  tag
            if ptag ==0 :
                ptag = record.samples[sample]['PS']
                record.samples[sample]['PS'] += tag

        if record.samples[sample].phased != True:
            if record.samples[sample]['PS'] == 0:
                if geno != (1,1):
                    tag += 1
                    ptag = 0
            if record.samples[sample]['GT'] != (1,1) and record.samples[sample]['GT'] != (0,0) and record.samples[sample]['GT'] != (2,2):
                ...
                if i > 1 and past_record.pos not in used_locus:
                    ...
                    if record.chrom == "HLA-DRB1" and past_record.pos >= 3880 and  past_record.pos <= 3700:
                        ...
                        block_boundaries.append(past_record.pos+max_allele_length)
            record.samples[sample].phased = True
            record.samples[sample]['PS'] = PS + tag
            

        if record.samples[sample]['PS'] != add_block and geno!=(1,1):
        ...
        #print (record.pos, record.samples[sample]['PS'], add_block)
        PS = record.samples[sample]['PS']
        #print("new:", record.chrom, record.pos, i, record.samples[sample].phased, record.samples[sample]['PS'], add_block)
...
def run_SpecHap():
    ...
    if args.tgs != 'NA':
        ...
        $bin/ExtractHAIRs --triallelic 1 --pacbio 1 --indels 1 --ref $ref --bam $outdir/$sample.tgs.sort.bam --VCF %s --out $outdir/fragment.$sample.file
        cat $outdir/fragment.$sample.file >> $outdir/fragment.all.file
        """%(args.tgs, outdir, sys.path[0], gene, hla_ref, args.thread_num, args.sample_id, gene_vcf)
    ...
    if args.nanopore != 'NA':
        ...
        """%(args.nanopore, hla_ref, outdir, sys.path[0], gene, args.thread_num, args.sample_id, gene_vcf)
...
def all_poss_block_link():
    ...
    for i in range(len(all_poss)):
        ...
        for z in range(1, 3):
            fasta = "%s/allele.%s.%s.fasta"%(outdir, z, gene)
            # poss_fasta = "%s/allele.raw.%s.%s.%s.fasta"%(outdir, i, z, gene)
            poss_fasta = "%s/%s.%s.%s.fasta"%(outdir, gene, i, z)
            # os.system("cp %s/allele.%s.%s.fasta %s/allele.raw.%s.%s.%s.fasta"%(outdir, z, gene, outdir, i, z, gene))
...
def select_poss():
    ...
    reph='perl %s/select.combination.pl -g %s -i %s -s %s -p Unknown'%(sys.path[0],gene,outdir,args.sample_id)        
    os.system(str(reph))    
...
def link_blocks():
    ...
    if args.focus_exon_flag == 1:
        ...
        if args.use_database == True:
            reph='python3 %s/map_block2_database.py %s %s'%(sys.path[0],gene,outdir)     
            os.system(str(reph))
...
def vcf2fasta(rephase_vcf):
    exon_bed = "%s/../db/all.gene.ref.exon.extend.txt"%(sys.path[0])
    exon_intervals = []
    f = open(exon_bed, 'r')
    for line in f:
        array = line.strip().split()
        ...
        for i in range(1, 3):
            if j == 0:
                fastq2 = '%s/../bin/samtools faidx %s %s:%s-%s | %s/../bin/bcftools consensus -H %s --mask %s %s\
                    >%s/allele.%s.%s.fasta'%(sys.path[0], hla_ref, gene, start, end, sys.path[0], i, mask_bed,\
...
if __name__ == "__main__":   
    if len(sys.argv)==1:
        ...
        if args.focus_exon_flag == 1:
            ...
            if gene == 'HLA-DRB1':
                # DRB1 contains long duplicates in the region 3900-3700 bp
