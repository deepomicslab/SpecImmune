...
class Pacbio_Binning():
    ...
    def map2db(self):
        ...
        $bin/minimap2 -t {parameter.threads} -O 50,60 -H -p 0.5 -N 100000 -a $ref $fq > $outdir/$sample.db.sam
        echo alignment done.
        """
    ...
    def read_bam(self):
        ...
        for read in self.bamfile:
            ...
            if read_obj.mismatch_rate > 0.1 or read_obj.match_rate == 0:
                continue
...
class Fasta():
    ...
    def vcf2fasta(self, gene):
        for index in range(2):
            ...
            $bin/minimap2 -t %s -a $gene_ref $outdir/$gene.%s.fq.gz | $bin/samtools view -bS -F 0x800 -| $bin/samtools sort - >$outdir/$gene.bam
            $bin/samtools index $outdir/$gene.bam


            longshot -F  --bam $outdir/$gene.bam --ref $gene_ref --out $outdir/$sample.$gene.longshot.vcf 
    ...
    def annotation(self):
        anno = """
        perl %s/annoHLA.pl -s %s -i %s -p %s -r whole -g %s -c %s
...
if __name__ == "__main__":   
    ...
    required.add_argument("-i", type=str, help="HLA,KIR,CYP",metavar="\b")
