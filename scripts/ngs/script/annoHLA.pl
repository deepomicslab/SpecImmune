...
my (%hash,%hashs,$ref,%hashp,%hashpp,%hashp2,%hashv);
...
close FIN;
open IN4, "$db/snp/merge.allele.snp.vcf" or die "$!\n";
while(<IN4>){
	chomp;
	my ($allele,$pos) = (split)[0,1];
	my $ge = (split /\*/,$allele)[0];
	my $k = "$ge\t$pos";
	$hashv{$k} .= "$allele\t";
}
close IN4;
...
foreach my $class(@genes){
	...
	elsif($wxs ne "tgs" && ($class eq "CYP3A5" || $class eq "CYP2A13"|| $class eq "CYP2C8" || $class eq "CYP2C9")) {$ref="$db/$gene_class/ref/split/$class.exon.fasta";}
	...
	if($wxs eq "exon"){
		...
		elsif($wxs ne "tgs" && ($class eq "CYP3A5" || $class eq "CYP2A13"|| $class eq "CYP2C8" || $class eq "CYP2C9")){
		...
		#my $ref="$db/split/$class.fasta";
	      ...
	      my %hash_max;
		      ...
		      next if(exists $hashg{$class} && $pop ne "nonuse" && $hashp{$d4} ==0);
                      my $scorel=$s1;
		      #print "$key\t$scorel\t$score\t$hash12{$key}\t$hash11{$key}\n";
		      if($scorel > $score){
                             $score = $scorel;
                             $gene = $key;
			     $size = $hash11{$key};
			     $hash_max{$scorel} .= "$gene\t";
                      }elsif($scorel == $score){
			      $hash_max{$scorel} .= "$gene\t";
	     ...
	     $hash{$tag} = $gene;
	     $hashs{$tag} = $score;
	     my @alleles = (split /\t/,$hash_max{$score});
	     my $bam = "$fadir/merge.$gene_class.bam";
             if($wxs eq "tgs"){$bam = "$fadir/$class.bam"}
	     if($#alleles >=1 && $class eq "CYP3A5"){
	     	system("$bin/blastn -query $fadir/allele.$i.$class.fasta -out $fadir/tmp/$class.blast.xml -db $db/split_ref/$class.fasta -outfmt 5 -penalty -1 -reward 1 -gapopen 4 -gapextend 1 -perc_identity 90 -qcov_hsp_perc 10");
		system("$bin/blastn -query $fadir/allele.$i.$class.fasta -out $fadir/tmp/$tag.blast.txt -db $db/split_ref/$class.fasta -outfmt 7 -penalty -1 -reward 1 -gapopen 4 -gapextend 1 -perc_identity 90 -qcov_hsp_perc 10");
		system("java -jar $bin/blastn2snp.jar < $fadir/tmp/$class.blast.xml|cut -f 1,5-9|cut -f 1,3,5,6 >$fadir/tmp/$tag.snp.vcf");
		my %hash_pos;
		open TT, "$fadir/tmp/$tag.snp.vcf" or die "$!\n";
		while(<TT>){
			chomp;
			next if(/^#/);
			my ($pos,$alt) = (split)[1,3];
			$hash_pos{$pos} = $alt;
		}
		close TT;
		foreach my $allele(@alleles){
			if($allele =~ /CYP3A5\*1/ && exists $hash_pos{"36708"}){$hash{$tag} = $allele}
		        if($allele =~ /CYP3A5\*3/ && exists $hash_pos{"12083"}){$hash{$tag} = $allele}
			if($allele =~ /CYP3A5\*6/ && exists $hash_pos{"19787"}){$hash{$tag} = $allele}
			if($allele =~ /CYP3A5\*7/ && exists $hash_pos{"32228"}){$hash{$tag} = $allele}
		}
