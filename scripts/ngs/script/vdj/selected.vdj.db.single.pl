...
my $db="$Bin/../db/IG_TR";
my $bin="$Bin/../bin";
#perl $0 read.vdj.file.txt read.assign_file.txt  ./test escc001n ESCC001N.sort.bam
#my $nn = "N"x20;
my $nn = "";

my ($infile, $afile, $outdir, $sample, $bam) = @ARGV;
my $bwa = "$bin/bwa";
...
my (%hash1,%hash2);
open F1, "$outdir/$sample.fq" or die "$!\tfq1\n";
while(<F1>){
     chomp;
     my $id = $_;
     $id =~ s/^@//;
...
`$bwa mem $outdir/$sample.VDJ.ref.fasta $outdir/$sample.fq |$samtools view -H >$outdir/header.sam`;
