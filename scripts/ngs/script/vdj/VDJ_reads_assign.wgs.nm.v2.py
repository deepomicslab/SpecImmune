...
def extract_reads(options, read12):
    print ('start assigning reads...')
    reads_file = '%s/read_names.txt'%(options.outdir)
    assign_file = '%s/%s.assign_file.txt'%(options.outdir,read12)
...
def select_VDJ_allele(options, readstats1, readstats2):
    flist = {}
    clist = defaultdict(int)
    gene_reads = defaultdict(list)
    assign_file1 = '%s/read1.assign_file.txt'%(options.outdir)
    assign_file2 = '%s/read2.assign_file.txt'%(options.outdir)
    ...
    with open(assign_file2) as f:
        for line in f:
            ...
            gene_reads[gene].append(read12)
    f.close()
    vdj_file = '%s/read.vdj.file.txt'%(options.outdir)
    out = open(vdj_file, 'w')
...
if __name__ == "__main__":
    ...
    readstats1 = extract_reads(options, "read1")
    readstats2 = extract_reads(options, "read2")
    #print(readstats)
    select_VDJ_allele(options, readstats1, readstats2)
