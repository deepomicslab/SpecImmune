...
gene=`echo $pos|cut -d ":" -f 1`
ref=$db/split_ref/$gene.fasta
...
python3 $dir/realignblast.py -i $bam -o $outdir/$sample.$class.realign.bam -r $outdir/rematch.total.read.format.txt
$sdir/samtools sort --threads $thread $outdir/$sample.$class.realign.bam > $outdir/$sample.$class.realign.sort.bam
#java -jar $sdir/picard.jar FixMateInformation I=$outdir/$sample.realign.sort.bam O=$outdir/$sample.realign.sort.fixmate.bam
$sdir/samtools index $outdir/$sample.$class.realign.sort.bam
