...
if [ $gene_class == "CYP" ];then
  database_prefix=$db/CYP/ref/CYP.select
...
if [ $gene_class == "CYP" ]; then
	python3 $dir/script/assign_reads_to_genes.py -a $gene_class -1 $fq1 -2 $fq2 -n $bin -o $outdir -d ${mini_score:-0.1} -b ${outdir}/${sample}.map_database.bam -nm ${nm:-2} -s 0 >$outdir/assign.log
else
...
if [ $gene_class == "KIR" ];then
   ...
   #perl $dir/script/filter.snp.pl $outdir/$sample.$gene_class.realign.vcf $outdir/$sample.$gene_class.realign2.vcf
   bam=$outdir/$sample.$gene_class.realign.sort.bam
fi

if [ $gene_class == "CYP" ];then
	mv $outdir/$sample.$gene_class.realign.vcf $outdir/$sample.$gene_class.realign.pre.vcf
	perl $dir/script/filter.snp.pl $outdir/$sample.$gene_class.realign.pre.vcf $outdir/$sample.$gene_class.realign.vcf
...
python3 $dir/script/mask_low_depth_region.py -c $bam.depth -o $outdir -w 20 -d ${mask_depth:-5} -f ${mask_exon:-True} -g $gene_class
