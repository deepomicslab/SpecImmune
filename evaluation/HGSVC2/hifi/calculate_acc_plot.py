...
plt.savefig("HGSVC_HIFI_performance_by_class_and_cutoff.svg", bbox_inches='tight', format="svg", dpi=600)  # Save the plot as a PDF
...
for class_name, genes in gene_classes.items():
    ...
    plt.tight_layout(pad=1)  # Adjust padding for better spacing
    plt.savefig(f"HGSVC_HIFI_performance_by_gene_and_cutoff_{class_name.replace(' ', '_')}.svg", bbox_inches='tight',  format="svg", dpi=600)  # Save the plot as a PDF
