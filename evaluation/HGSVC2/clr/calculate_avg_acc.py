...
genes_for_speclong = [gene for gene in pd.concat([df1['Gene'], df2['Gene'], df3['Gene']]).unique() 
...
# Step 4: Set the chosen depth threshold
...
# Step 6: Calculate detailed accuracy for each gene in each DataFrame based on the chosen depth
for df, df_name in zip(dfs, df_names):
    ...
    for gene in relevant_genes:
        ...
        # Filter the DataFrame for the valid samples (read depth < chosen_depth)
        ...
        # Store the result for < chosen_depth in plot_data
...
# excluding genes with no valid data from the statistics.
for depth_category in plot_data:
    print(f"\nResults for depth category: {depth_category}")
    for df_name in df_names:
        print(f"\nSoftware: {df_name}")
        valid_accuracies = []   # List only for genes with valid data
        total_matches_sum = 0   # Total correct alleles (only for valid genes)
        total_alleles_sum = 0   # Total alleles processed (only for valid genes)
        ...
        for entry in plot_data[depth_category][df_name]:
            ...
            total_matches_gene = gene_data['Match'].sum()
            total_alleles_gene = gene_data['Total'].sum()
            ...
            if accuracy is not None:
                ...
                print(f"  Gene: {gene}, Accuracy: No valid data (gene not included in stats)")
