...
pdf(file="figures/count_heatmap.pdf", width=6, height=5, onefile=FALSE)


df<-read.table("pop_occur_freq.csv", sep=",", header=TRUE)

## assign x order
df$Group <- factor(df$Group, levels = c('CDX', 'CHB', 'CHS', 'JPT', 'KHV', 'BEB', 'GIH', 'ITU', 'PJL', 'STU', 'ACB', 'ASW', 'ESN', 'GWD', 'LWK', 'MSL', 'YRI', 'CEU', 'FIN', 'GBR', 'IBS', 'TSI', 'CLM', 'MXL', 'PEL', 'PUR'))
...
p1<-ggplot(data = df, aes(y=Group, x=Gene, fill=Freq)) + 
  geom_tile()+
