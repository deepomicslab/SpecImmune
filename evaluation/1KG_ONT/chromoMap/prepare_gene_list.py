...
def read_all():
    all_data = []
    hla_list = read_bed("../../../gene_dist/HLA.gene.bed")
    ## add 'HLA' to each element
    for i in range(len(hla_list)):
        hla_list[i].append('HLA')
    all_data += hla_list
    kir_list = read_bed("../../../gene_dist/KIR.gene.bed")
    for i in range(len(kir_list)):
        kir_list[i].append('KIR')
    all_data += kir_list

    cyp_list = read_bed("../../../gene_dist/CYP.gene.bed", 'CYP')
    for i in range(len(cyp_list)):
        cyp_list[i].append('CYP')
    all_data += cyp_list

    ig_tr_list = read_bed("../../../gene_dist/IG_TR.gene.bed")
    for i in range(len(ig_tr_list)):
        ig_tr_list[i].append('IG_TR')
    all_data += ig_tr_list
    # handle_data(all_data)
    print (len(all_data), all_data[:5])
    ## transform the data to a dataframe
    df = pd.DataFrame(all_data, columns=['gene', 'chr', 'start', 'end', 'strand', 'family'])
    ## write the data to a csv file
    df.to_csv("SpecImmune_full_gene_list.csv", index=False)
