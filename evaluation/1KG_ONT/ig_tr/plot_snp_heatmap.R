...
pdf(file="vdj_pop_hete_heatmap.pdf", width=12, height=5, onefile=FALSE)


df<-read.table("vdj_pop_hete_freq.csv", sep=",", header=TRUE)
...
p1<-ggplot(data = df, aes(x=Gene, y=Pop, fill=hete_freq)) + 
  geom_tile()+
   theme(axis.text.x = element_text(size = 8))+
   scale_x_discrete(guide = guide_axis(angle = 90))+
 geom_tile(color = "white")+
 scale_fill_gradient2(low = "blue", high = "red", mid = "white",midpoint = 0.1,
