...
pdf(file="hla/hla_count.pdf", width=8, height=4, onefile=FALSE)
df<-read.table("hla/hla_gene_allele.csv", sep=",", header=TRUE)
...
ggplot(data=df, aes(x=Gene, y=Count, fill=Type)) +
  geom_bar(stat="identity", position=position_dodge())+scale_fill_brewer(palette="Dark2") +theme_classic()+
