...
# Step 1: 读取基因距离数据
file_path = 'distance.nm.min100.csv'
...
superpop_file = 'distance.sub.nm.min100.samples'
...
for i, row in data.iterrows():
    ...
    dist = gene_distances.mean()  # 使用平均值作为距离
...
# 使用 red to blue 的自定义 colormap
cmap = sns.diverging_palette(250, 10, as_cmap=True)  # 从红色到蓝色
...
ax = sns.heatmap(
    ...
    square=True,  # 正方形单元格
    vmin=sorted_distance_matrix.min(),  # 最小值
    vmax=sorted_distance_matrix.max(),  # 最大值
    center=50  # 设置中点为 5000
)
