...
REFERENCE_ALLELE_PATH = '/gpfs1/scratch/ResearchGroups/cs_shuaicli/wxd/app/SpecLong/db/HLA_analysis_dir/{gene}.fasta'  # 假设每个基因都有一个参考等位基因文件


def generate_bam_and_get_divergency_nm(allele1, reference_allele, bam_file, threads=4):
    """
    使用 minimap2 比对等位基因与参考等位基因，并返回比对的差异值 (使用 NM 标签)。
    """
    # 运行 minimap2 进行比对并生成 BAM 文件
    minimap2_cmd = f"minimap2 -ax asm10 {reference_allele} {allele1} -t {threads} | samtools view -bS -F 0x800 - | samtools sort > {bam_file}"
    subprocess.run(minimap2_cmd, shell=True, check=True, stderr=subprocess.DEVNULL)
...
def compute_sample_to_ref_divergency_vector(sample, bam_output_folder):
    """
    计算样本与参考等位基因的差异向量（针对每个基因），返回差异值。
    """
    divergency_vector = []
    sample_folder = os.path.join(args.path, sample, sample, 'Sequences')
    
    for gene in GENES:
        s1_allele1 = os.path.join(sample_folder, f'HLA.allele.1.{gene}.fasta')
        s1_allele2 = os.path.join(sample_folder, f'HLA.allele.2.{gene}.fasta')
        ref_allele = REFERENCE_ALLELE_PATH.format(gene=gene)

        dv_s1_ref = []
        # 比对等位基因1和等位基因2分别与参考等位基因
        dv_s1a1_ref = generate_bam_and_get_divergency_nm(s1_allele1, ref_allele, os.path.join(bam_output_folder, f'{sample}_{gene}_s1a1_ref.bam')) or 0
        dv_s1a2_ref = generate_bam_and_get_divergency_nm(s1_allele2, ref_allele, os.path.join(bam_output_folder, f'{sample}_{gene}_s1a2_ref.bam')) or 0
        dv_s1_ref.append(dv_s1a1_ref)
        dv_s1_ref.append(dv_s1a2_ref)

        # 将最小差异值作为该基因的差异值
        divergency_vector.append(min(dv_s1_ref))
    
    return divergency_vector

def compute_distance_matrix_to_ref(sample_names, output_csv, bam_output_folder):
    """
    计算所有样本与参考等位基因之间的距离矩阵并保存到 CSV。
    """
    num_samples = len(sample_names)
    num_genes = len(GENES)
    distance_matrix = np.zeros((num_samples, num_genes))
    
    os.makedirs(bam_output_folder, exist_ok=True)
    
    for i, sample in enumerate(sample_names):
        tqdm.write(f"Processing {sample}... ({i + 1}/{num_samples})")
        divergency_vector = compute_sample_to_ref_divergency_vector(sample, bam_output_folder)
        distance_matrix[i, :] = divergency_vector
    
    # 保存结果到 CSV 文件
    with open(output_csv, 'w') as f:
        header = "Sample," + ",".join(GENES) + "\n"
        f.write(header)
        
        for i, sample in enumerate(sample_names):
            divergency_vector = distance_matrix[i, :]
            line = f"{sample}," + ",".join(map(str, divergency_vector)) + "\n"
            f.write(line)
...
if __name__ == '__main__':
    ...
    # 计算选择样本与参考等位基因的距离矩阵
    compute_distance_matrix_to_ref(selected_samples, args.output, args.bam_output)