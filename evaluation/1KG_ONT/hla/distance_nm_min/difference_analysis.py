...
# Step 1: 读取基因距离数据
file_path = 'distance.nm.min.csv'  # 替换为你的文件路径
data = pd.read_csv(file_path)

# Step 2: 读取 Superpopulation 信息文件
superpop_file = 'distance.sub.nm.min.samples'  # 替换为你的文件路径
...
for index, row in data.iterrows():
    ...
    # 提取基因距离（从第3列开始）
    gene_distance = np.median(row[2:]) # 假设基因距离列是从第3列开始的，取均值
...
plt.savefig('distance_distribution.pdf', format='pdf', bbox_inches='tight', dpi=600)
