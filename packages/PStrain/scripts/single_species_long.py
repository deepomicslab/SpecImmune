...
"""
python /home/wangshuai/softwares/SpecLong/packages/PStrain/scripts/single_species_long.py \
    -b /mnt/d/HLAPro_backup/Nanopore_optimize/data/hg38/HG00732.bam  \
    -v  /mnt/d/HLAPro_backup/Nanopore_optimize/data/hg38/HG00732.vcf  \
    -o /mnt/d/HLAPro_backup/Nanopore_optimize/data/hg38/test/ \
    -n hg00732
"""
...
required.add_argument("-n", "--sample",help="prefix.",dest='prefix',metavar='')
...
optional.add_argument("--snp_dp",help="The minimum depth of SNPs to be considered in strain profiling step (default is 5).",\
    dest='snp_dp',metavar='',default=5, type=int)
#added parameters
...
def output(outdir,final_alpha,seq_list,to_sort,snp_list):
    if not args.prefix:
        ra_file_name = outdir+'/strain_RA.txt'
        seq_file_name = outdir+'/strain_seq.txt'
    else:
        ra_file_name = outdir+'/'+args.prefix+'_hap_freq.txt'
        seq_file_name = outdir+'/'+args.prefix+'_hap_seq.txt'

    ra_file=open(ra_file_name,'w')
